...
def run_scanner(mount_path, host2scan, workers, yaramode, rp_id, rp_ts, rp_status, db_path, csv_path=None):
...
# Run Mount & Scan
def do_mount_scan(token, scanhost, local_ip, rp_id, hostname, use_iscsi, workers, yaramode, args, rp_ts, rp_status, db_path, csv_path):
    mount_body = {
    ...
    if use_iscsi:
        run_iscsi_scan(mount_id, mount_info.get("info", {}), hostname, workers, yaramode, args, rp_id, rp_ts, rp_status, args.db, args.csv)
    else:
...
def main():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.maxhosts) as executor:
                ...
                for host in hostnames:
                    ...
                    if rp.get("data"):
                        ...
                        futures.append(executor.submit(do_mount_scan, token, scanhost, local_ip, restore_id, host, args.iscsi, args.workers, args.yaramode, args, ts, rp_status, args.db, args.csv))
    ...
    if args.host2scan:
        ...
        do_mount_scan(token, scanhost, local_ip, rp_id, args.host2scan, args.iscsi, args.workers, args.yaramode, args, ts, rp_status, args.db, args.csv)
        return token
